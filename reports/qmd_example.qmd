...
![Horse populations for all provinces in Canada from 1906 - 1972](../results/horse_pops_plot.png){#fig-horse-pop-img}

We can see from @fig-horse-pop-img that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-horses-std.

```{python}
#| label: tbl-horses-std
...
Additionally, note that in @tbl-horses-std we consider the sample standard deviation of the number of horses during the same time span as @fig-horse-pop-img.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){#fig-horse-std}
